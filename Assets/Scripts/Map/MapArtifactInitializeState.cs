...
public class MapArtifactInitializeState : StateBase {
    ...
    override public bool OnBeforeMain()
    {
		...
		if (dungeonState == "ArtifactRewardWait")
		{
			MapDataCarrier.Instance.ArtifactList.AddRange(PlayerPrefsManager.Instance.GetLotArtifactList());
			for (int i = 0; i < 3; i++)
			{
				MasterArtifactTable.Data data = MasterArtifactTable.Instance.GetData(MapDataCarrier.Instance.ArtifactList[i].Id);
